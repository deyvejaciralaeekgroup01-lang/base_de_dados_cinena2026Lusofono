...
-- MovieCountry: distribuir filmes por país (garantir contagens por continente)
-- Europe: movies 1-12
...
-- ----------------------------------------------------------------------------
PRINT '4.1 - Videos do genero Action:'; -- erro, só tem 3 filmes de ação
SELECT 
    v.movieId, 
    v.movieName, 
    v.movieReleaseDate
...
-- ----------------------------------------------------------------------------
PRINT '4.5 - Videos de Action europeus lançados em Dezembro:'; -- erro
SELECT 
    v.movieId, 
    v.movieName, 
    v.movieReleaseDate, 
    ...
    g.genreName
FROM Movies v
INNER JOIN genres_movies vg ON v.movieId = vg.movieId              -- Para filtrar por género
INNER JOIN Genres g ON vg.genreId = g.genreId                    -- Obtém nome do género
INNER JOIN MovieCountry vc ON v.movieId = vc.movieId            -- Para filtrar por país
INNER JOIN Country c ON vc.countryId = c.countryId              -- Obtém nome do país
INNER JOIN Continent ct ON c.continentId = ct.continentId       -- Para filtrar por continente
WHERE g.genreId = 3817                                 -- Condição 1: Género Action
  AND ct.continentId = 1                               -- Condição 2: Continente Europa
  AND MONTH(v.movieReleaseDate) = 12;                           -- Condição 3: Mês Dezembro
GO
...
Audit com a informacao da ação executada (INSERT), do
